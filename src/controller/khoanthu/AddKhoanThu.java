...
public class AddKhoanThu implements Initializable {
    ...
    public void addKhoanThu(ActionEvent event) throws ClassNotFoundException, SQLException {
        ...
        if (dateHanNop.getValue() == null) {
            Alert alert = new Alert(Alert.AlertType.WARNING, "Hãy nhập ngày hạn nộp", ButtonType.OK);
            alert.setHeaderText(null);
            alert.showAndWait();
            return;
        }
        ...
        new KhoanThuService().add(new KhoanThuModel(maKhoanThuInt, tenKhoanThuString, soTienDouble, loaiKhoanThu, datehannop));
